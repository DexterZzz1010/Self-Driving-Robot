    ...
    while (ros::ok()){
        ...
        if (flag == true && !img_raw.empty())
        {
           ...
           int center = frame.cols / 2;
            ...
            cv::Point left_pt(0, 0);  // 初始化车道线点为中心点
            ...
            for (int y = 0; y < image.rows; y+=50) {  // 遍历图像每一行
                // cv::Vec3b pixel = image.at<cv::Vec3b>(cv::Point(0,0));  // 获取中心点的 RGB 值
                // cv::Vec3b yellow (0, 255, 255);
                points.push_back(left_pt);
                // if (pixel == yellow) {  // 如果 RGB 值为 (0, 255, 255)，说明找到了车道线
                //     // cv::Point2f left_pt = cv::Point2f(center, y);  // 更新车道线点
                //     // left_pt = cv::Point(center, y);  // 更新车道线点
                //     // left_pts.emplace_back({static_cast<float>(center), static_cast<float>(y)}); // 保存车道线点
                //     // points.push_back(left_pt); // 保存车道线点
                //     // left_pts[count]=left_pt;
                //     // left_pts.push_back(left_pt); // 保存车道线点
                //     continue;  // 继续查找下一行
                // }
                for (int x = center ; x >= 0; x--) {  // 从中心点向左查找
                    // cv::Vec3b pixel = image.at<cv::Vec3b>(cv::Point(y, x));  // 获取当前点的 RGB 值
                    ...
                    left_pts.push_back(left_pt);  // 保存车道线点
                    if (h >= 20 && h <= 40 && s >= 100 && s <= 255 && v >= 100 && v <= 255) {
                    // 像素点为黄色
                    left_pts.push_back(left_pt); 
                    } 
                    // if (pixel == yellow) {  // 如果 RGB 值为 (0, 255, 255)，说明找到了车道线
                    //     // left_pt = cv::Point2f(x, y);  // 更新车道线点
                    //     // left_pts.push_back(left_pt);  // 保存车道线点
                    //     break;  // 停止查找
                    // }
                }
            ...
            // // 从垂直中线向右查找车道线
            // std::vector<cv::Point2f>right_pts;  // 保存车道线点的容器
            // cv::Point right_pt(center, 0);  // 初始化车道线点为中心点
            // for (int y = 0; y < image.rows; y+=50) {  // 遍历图像每一行
            //     cv::Vec3b pixel = image.at<cv::Vec3b>(cv::Point(center, y));  // 获取中心点的 RGB 值
            //     if (pixel == cv::Vec3b(0, 255, 255)) {  // 如果 RGB 值为 (0, 255, 255)，说明找到了车道线
            //         right_pt = cv::Point(center, y);  // 更新车道线点
            //         right_pts.push_back(right_pt);  // 保存车道线点
            //         continue;  // 继续查找下一
            //     }
            //     for (int x = center + 1; x < image.cols; x++) { // 从中心点向右查找
            //         cv::Vec3b pixel = image.at<cv::Vec3b>(cv::Point(x, y)); // 获取当前点的 RGB 值
            //         if (pixel == cv::Vec3b(0, 255, 255)) { // 如果 RGB 值为 (0, 255, 255)，说明找到了车道线
