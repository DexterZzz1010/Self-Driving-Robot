...
float LaneDetection::Draw(cv::Mat& mat, cv::Mat& mat_topview, CameraModel& camera)
{
    /*** Draw on NormalView ***/
    /* draw points */
    ...
    */
    float center = mat_topview.cols/2;
    float err = 0 ;
    if (topview_line_list_[1].size()!=0 && topview_line_list_[2].size()!=0){
        cv::Vec4f left_line, right_line;
        cv::fitLine(topview_line_list_[1], left_line, cv::DIST_L2, 0, 0.01, 0.01);  // 拟合左车道线
        cv::fitLine(topview_line_list_[2], right_line, cv::DIST_L2, 0, 0.01, 0.01);  // 拟合右车道线
    ...
    for (int32_t line_index = 0; line_index < static_cast<int32_t>(line_coeff_list_.size()); line_index++) {
        ...
        if (line_valid_list_[line_index]) {
            for (float z = 0; z < kLineFarthestPointMeter[line_index]; z += kLineIntervalMeter) {
                ...
                cv::line(mat_topview, image_point_0, image_point_1, GetColorForLine(line_index), 3);
            }
        }
    }
    return err;
